...
- 页面默认使用简体中文，并通过应用语言设置支持简体中文与 English；用户内容保持原文，不随界面语言改写。
...
- 仓库内所有运行时下载、发布、文档和 GitHub 操作地址统一使用 `qxryz/workflowgenerator`；`basketikun/infinite-canvas` 只允许出现在 README 致谢或第三方来源声明中。不存在的目录、插件和发布链路不得继续保留工作流或操作文档。
