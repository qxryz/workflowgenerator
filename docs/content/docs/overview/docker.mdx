---
title: Docker 部署
description: 使用 Docker Compose 部署 WorkflowGenerator 网页兼容模式
---
...
Docker 运行的是网页兼容模式，不包含桌面版的 Tauri 原生数据层。画布、素材、生成记录和模型渠道配置保存在访问者当前浏览器的 IndexedDB/localForage 中；第三方提示词由前端直连拉取，不需要为主应用挂载数据目录。清理浏览器站点数据会删除这些本地内容，正式使用前请导出备份或配置 WebDAV。
...
services:
  app:
    image: ghcr.io/qxryz/workflowgenerator:latest
    ports:
      - "3000:3000"
    environment:
...
docker run -d -p 3000:3000 \
  ...
  ghcr.io/qxryz/workflowgenerator:latest
```
