import { openAiAudioVoiceOptions } from "./audio-defaults";
...
export function openAiAudioVoiceRequestValue(value: string): string | { id: string } {
    const voice = normalizeAudioVoiceValue(value);
    return audioVoiceOptions.some((item) => item.value === voice) ? voice : { id: voice };
}
...
export function audioVoiceLabel(value: string) {
    const voice = normalizeAudioVoiceValue(value);
    return audioVoiceOptions.find((item) => item.value === voice)?.label || `自定义 · ${voice}`;
}
