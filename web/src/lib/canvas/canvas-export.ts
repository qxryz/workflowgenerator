...
export async function exportCanvasProjects(projects: CanvasProject[], fileName = "WorkflowGenerator") {
    ...
    const exportedProjects = await Promise.all(
        projects.map(async (project) => {
            ...
            return { project: portableCanvasProject(project), files };
        }),
    );
...
export async function exportCanvasNodes(nodes: CanvasNodeData[], fileName = "画布元素") {
    ...
    await Promise.all(
        nodes.map(async (node) => {
            ...
            zipFiles.push({ name: uniqueName(title, "json"), data: JSON.stringify(portableCanvasNode(node), null, 2) });
        }),
    );
