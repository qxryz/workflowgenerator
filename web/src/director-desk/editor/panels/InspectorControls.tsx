import { Children, isValidElement, useCallback, useEffect, useRef, useState, type KeyboardEvent, type MouseEvent, type ReactNode } from "react";
...
type RangeNumberFieldProps = {
    ...
    normalizeNumberValue?: (value: string) => string;
    min: string | number;
    max: string | number;
    step: string | number;
};
...
export function InspectorPanel({ title, ariaLabel, tabs, className, children, footer }: { title: string; ariaLabel: string; tabs?: InspectorTab[]; className?: string; children: ReactNode; footer?: ReactNode }) {
    return (
        <section className={`panel-card right-inspector${className ? ` ${className}` : ""}`} aria-label={ariaLabel}>
            ...
            {tabs ? (
                <div className="tab-row right-inspector-tabs" role="tablist" aria-label={`${title}面板标签`}>
                    {tabs.map((tab) => (
                        <button key={tab.label} className="right-inspector-tab-button" type="button" aria-pressed={tab.active} onClick={tab.onClick}>
                            {tab.label}
                        </button>
                    ))}
                </div>
...
export function InspectorTextField({ label, ariaLabel, value, onChange, type = "text", step, min, max }: TextFieldProps) {
    const { beginInteraction, endInteraction } = useUndoBatchInteraction();

    return (
        <label className="inspector-field">
            <span className="inspector-field-label">{label}</span>
            <input aria-label={ariaLabel} className="inspector-text-input" max={max} min={min} step={step} type={type} value={value} onChange={(event) => onChange(event.currentTarget.value)} onBlur={endInteraction} onFocus={beginInteraction} />
        </label>
    );
}

export function InspectorSelectField({ label, ariaLabel, value, onChange, children, options }: { label: string; ariaLabel: string; value: string; onChange: (value: string) => void; children?: ReactNode; options?: InspectorSelectOption[] }) {
    ...
    return (
        <div className="inspector-field inspector-select-field">
            ...
            <div className="inspector-dropdown" ref={dropdownRef}>
                <button aria-expanded={isOpen} aria-haspopup="listbox" aria-label={ariaLabel} className="inspector-dropdown-trigger" type="button" onClick={() => setIsOpen((current) => !current)} onKeyDown={handleTriggerKeyDown}>
                ...
                {isOpen ? (
                    <div aria-label={ariaLabel} className="inspector-dropdown-menu" role="listbox">
                        {resolvedOptions.map((option) => {
                            ...
                            return (
                                <button aria-selected={isSelected} className={`inspector-dropdown-option${isSelected ? " is-selected" : ""}`} disabled={option.disabled} key={option.value} role="option" type="button" onClick={() => selectOption(option)}>
...
function InspectorAxisInput({ control }: { control: AxisControl }) {
    ...
    return (
        <div className={`inspector-axis-input${isDragging ? " is-dragging" : ""}`}>
            <button aria-label={`${control.ariaLabel} 拖动调整`} className="inspector-axis-prefix" type="button" onKeyDown={handlePrefixKeyDown} onMouseDown={handlePrefixMouseDown}>
                {control.axis}
            </button>
            <input
...
export function InspectorRangeNumberField({ label, rangeAriaLabel, numberAriaLabel, value, onValueChange, onRangeChange, onNumberChange, onNumberBlur, normalizeNumberValue, min, max, step }: RangeNumberFieldProps) {
    ...
    function normalizeDraft(rawValue: string) {
        if (!normalizeNumberValue || rawValue.trim() === "" || !Number.isFinite(Number(rawValue))) return null;
        ...
        <div className="inspector-field inspector-range-field">
            ...
            <div className="inspector-range-row">
                ...
                <input
                    ...
                    min={min}
                    step={step}
                    inputMode={normalizeNumberValue ? "decimal" : undefined}
                    type={normalizeNumberValue ? "text" : "number"}
                    value={normalizeNumberValue ? (numberDraft ?? String(value)) : value}
                    onBlur={(event) => {
                        if (normalizeNumberValue) {
                            const nextValue = normalizeDraft(numberDraft ?? event.currentTarget.value);
                            if (nextValue !== null) {
                                (onNumberChange ?? onValueChange)(nextValue);
                                onNumberBlur?.(nextValue);
                            }
                            setNumberDraft(null);
                        } else {
                    ...
                    onChange={(event) => {
                        ...
                        if (!normalizeNumberValue) {
                            (onNumberChange ?? onValueChange)(rawValue);
                            return;
                        }
                        setNumberDraft(rawValue);
                        const nextValue = normalizeDraft(rawValue);
                        if (nextValue !== null) {
                    ...
                    onFocus={() => {
                        if (normalizeNumberValue) setNumberDraft(String(value));
                        beginInteraction();
                    }}
                />
            </div>
    ...
    return (
        <label className="inspector-field">
            ...
            <div className="inspector-color-row">
                <input aria-label={colorAriaLabel} className="inspector-color-swatch" type="color" value={value} onChange={(event) => onColorChange(event.currentTarget.value)} onBlur={endInteraction} onFocus={beginInteraction} />
                <input aria-label={hexAriaLabel} className="inspector-text-input inspector-color-hex" value={value} onChange={(event) => onHexChange(event.currentTarget.value)} onBlur={endInteraction} onFocus={beginInteraction} />
            </div>
        </label>
    );
}
