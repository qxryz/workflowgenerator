...
export function PropPanel() {
  const prop = useDirectorStore((state) => {
    ...
    if (selected.kind === "prop" || selected.geometryType) return selected;
  ...
  return (
    <InspectorPanel
      title={prop.geometryType ? "几何体" : "模型"}
      ariaLabel={`${prop.geometryType ? "几何体" : "模型"}右侧属性面板`}
      className="prop-inspector"
    >
      ...
      />
      <SelectionDeleteControls targetObjectId={prop.id} />
    </InspectorPanel>
  );
}
