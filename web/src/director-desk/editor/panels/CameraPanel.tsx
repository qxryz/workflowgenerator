import { Camera, Download, Eye, Images, Save, Trash2, X, ZoomIn, ZoomOut } from "lucide-react";
...
import { exportDirectorDeskCaptureToHost, getDirectorCaptureDestinationLabel, isEmbeddedDirectorDesk, postDirectorDeskCapturesToHost } from "../io/hostBridge";
...
import { SelectionDeleteControls } from "./CharacterDeleteControls";

const VIEWER_ZOOM_MIN = 0.25;
const VIEWER_ZOOM_MAX = 5;
const VIEWER_ZOOM_STEP = 0.25;

function capturePayload(capture: DirectorCameraCapture) {
  return {
    id: capture.id,
...
export function CameraPanel() {
  ...
  const captureDestinationLabel = getDirectorCaptureDestinationLabel();
  ...
  const targetSelectValue =
    currentCamera.targetMode === "object" && currentCamera.targetObjectId
      ...
      : "manual";
  const cameraObjectId = objects.find(
    (item) => item.kind === "camera" && item.linkedCameraId === currentCamera.id
  )?.id;
  ...
  const saveCapture = useCallback((capture: DirectorCameraCapture) => {
    postDirectorDeskCapturesToHost([capturePayload(capture)]);
  }, []);

  const saveAllCaptures = useCallback(() => {
    postDirectorDeskCapturesToHost(
      cameraCaptureGroups.flatMap((group) =>
        group.captures.map(capturePayload)
      )
    );
  }, [cameraCaptureGroups]);

  const exportCapture = useCallback((capture: DirectorCameraCapture) => {
    if (isEmbeddedDirectorDesk()) {
      downloadDataUrl(capture.dataUrl, `${capture.name}.png`);
      return;
    }
    exportDirectorDeskCaptureToHost(capturePayload(capture));
  }, []);
            ...
            <div key={capture.id} className="camera-capture-card">
              ...
              >
                ...
                >
                  ...
                  <button
                    aria-label={`${captureDestinationLabel} ${capture.name}`}
                    className="camera-capture-action"
    ...
    return (
      <div className="camera-capture-overview-footer">
        ...
        <button
          ...
          type="button"
          onClick={saveAllCaptures}
        >
          <Save aria-hidden="true" data-testid="camera-capture-send-icon" size={14} strokeWidth={1.9} />
          <span>{captureDestinationLabel}</span>
        </button>
      </div>
    );
  }
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="camera-capture-viewer-tool"
            type="button"
            onClick={() => exportCapture(viewerCapture)}
          >
  ...
  return (
    ...
    >
      ...
      {cameraObjectId ? <SelectionDeleteControls targetObjectId={cameraObjectId} /> : null}
      {renderViewer()}
    </InspectorPanel>
  );
}
