...
export function ObjectTreePanel() {
  ...
  function beginRowDeletion(item: SceneTreeItem) {
    if (item.crowdId) {
      selectCrowd(item.crowdId);
    } else {
      selectObject(item.id);
  ...
  function confirmRowDeletion(item: SceneTreeItem) {
    if (item.crowdId) {
      selectCrowd(item.crowdId);
    } else {
      selectObject(item.id);
  ...
  return (
    <section className="panel-card object-tree-panel">
            ...
            <section key={group.key} className="object-tree-group" role="group" aria-label={`${group.title}分组`}>
              ...
              <ul className="object-list">
                {group.items.map((item) => {
                  ...
                  const canDeleteFromRow = true;
                  const confirmingDeletion = pendingDeleteId === item.id;
                  const deleteConfirmationTitleId = `delete-${item.id}-title`;
                  const deleteConfirmationDescriptionId = `delete-${item.id}-description`;

                  return (
                    <li key={item.id} className="object-list-item">
                      <div
                        className={`object-row${selected ? " is-selected" : ""}${item.crowdId ? " object-row-crowd" : ""}${canDeleteFromRow ? " has-delete-action" : ""}`}
                        role="treeitem"
                      ...
                      >
                        ...
                        {canDeleteFromRow ? (
                          <button
                            className="object-row-delete-button"
                            type="button"
                      ...
                      {confirmingDeletion ? (
                        <div
                          className="object-row-delete-confirmation"
                          role="alertdialog"
                          aria-labelledby={deleteConfirmationTitleId}
                          aria-describedby={deleteConfirmationDescriptionId}
                        >
                          <div className="object-row-delete-confirmation-copy">
                            <strong id={deleteConfirmationTitleId}>删除“{item.name}”？</strong>
                            <span id={deleteConfirmationDescriptionId}>
                              {item.crowdId
                                ? `其中的 ${item.objectIds.length} 个角色会一起删除。`
                                : item.icon === "camera"
                                  ? "该机位及其镜头设置会从场景中删除。"
                                  : item.icon === "model"
                                    ? "该模型实例会从场景中删除。"
                          ...
                          <div className="object-row-delete-confirmation-actions">
                            <button type="button" autoFocus onClick={() => setPendingDeleteId(null)}>
                              取消
                            </button>
                            <button
                              className="object-row-delete-confirm-button"
                              type="button"
                              onClick={() => confirmRowDeletion(item)}
                            >
                              确认删除
                            </button>
                          </div>
      ...
      <p className="object-delete-hint">
        <Trash2 aria-hidden="true" size={13} strokeWidth={1.8} />
        <span>点击垃圾桶删除，或选中后按 Delete</span>
      </p>
    </section>
  );
}
