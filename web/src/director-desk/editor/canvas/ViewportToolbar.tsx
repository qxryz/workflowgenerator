...
export function ViewportToolbar({
  ...
  const [activeModelLibraryCategoryId, setActiveModelLibraryCategoryId] =
    useState<ModelLibraryCategoryId>("my-models");
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => setImportNotice(null), 4500);
  ...
  async function handleLocalModelChange(
    ...
    try {
      ...
      if (failedFiles.length === 0) {
        const destination = addToScene ? "添加到场景" : "保存到“我的模型”";
        const subject =
          files.length === 1 ? `“${files[0]?.name ?? "模型"}”` : `${files.length} 个模型`;
        setImportNotice({
          kind: "success",
          message: `已将${subject}${destination}`,
        });
      } else if (importedCount > 0) {
        setImportNotice({
          kind: "error",
          message: `已导入 ${importedCount} 个模型，另有 ${failedFiles.length} 个文件未能导入。`,
        });
      } else {
        setImportNotice({
          kind: "error",
          message: "模型导入失败，请换一个 FBX 或 OBJ 文件重试。",
        });
      }
    } finally {
      input.value = "";
  ...
  async function handlePanoramaChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImportNotice({
        kind: "success",
        message: `已导入全景图“${file.name}”`,
      });
    } catch {
      setImportNotice({
        kind: "error",
        message: "全景图导入失败，请换一张 JPG、PNG 或 WEBP 图片重试。",
      });
    } finally {
      input.value = "";
    }
        ...
        {importNotice ? (
          <div
            ...
            style={{
              alignItems: "center",
              background:
                importNotice.kind === "error" ? "rgb(127 29 29 / 0.78)" : undefined,
              borderColor:
                importNotice.kind === "error" ? "rgb(248 113 113 / 0.34)" : undefined,
              bottom: "calc(100% + 10px)",
              boxShadow: "var(--ui-shadow-panel)",
              color: importNotice.kind === "error" ? "rgb(254 202 202)" : undefined,
              display: "flex",
              gap: 8,
              left: "50%",
