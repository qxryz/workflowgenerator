...
import { clampPoseControlValue, degreesToRadians } from "../mannequin/mannequinPose";
...
function radians(controls: Record<string, number>, key: string, bodyType?: CharacterBodyType) {
  return degreesToRadians(clampPoseControlValue(key, controls[key] ?? 0, bodyType));
}
...
function ue4SpineRotation(
  ...
  return [
    radians(controls, `${prefix}.yaw`, bodyType),
    radians(controls, `${prefix}.roll`, bodyType),
    -radians(controls, `${prefix}.pitch`, bodyType),
  ];
}
...
function ue4ShoulderRotation(
  ...
  return [
    radians(controls, `${prefix}.twist`, bodyType),
    degreesToRadians(clampPoseControlValue(`${prefix}.spread`, spread, bodyType)),
    -radians(controls, `${prefix}.pitch`, bodyType),
  ];
}
...
function ue4LimbBendRotation(
  ...
  return [0, 0, -radians(controls, key, bodyType)];
}
