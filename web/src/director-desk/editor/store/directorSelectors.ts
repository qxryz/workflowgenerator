...
export function selectRightPanelKind(state: DirectorState): RightPanelKind {
    ...
    const selectedAsset = selected?.assetRefId ? state.project.assets.find((asset) => asset.id === selected.assetRefId) : undefined;
    if (selected?.kind === "character") return "character";
    if (selected?.kind === "prop" || selected?.geometryType || selectedAsset?.sourceType === "model") return "prop";
