...
export function CanvasNodePromptPanel({ node, isRunning, onPromptChange, onConfigChange, onGenerate, onStop, mentionReferences = [], onImageSettingsOpenChange, modeOverride }: CanvasNodePromptPanelProps) {
    ...
    const readyReferences = mentionReferences.filter((reference) => reference.ready && reference.active);
    const videoReferenceCounts = {
        images: readyReferences.filter((reference) => reference.kind === "image").length,
        videos: readyReferences.filter((reference) => reference.kind === "video").length,
        audios: readyReferences.filter((reference) => reference.kind === "audio").length,
    };
                ...
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    {mode === "image" ? (
                        ...
                        <>
                            ...
                            <CanvasVideoSettingsPopover config={config} referenceCounts={videoReferenceCounts} buttonClassName="!h-10 !max-w-[170px] !justify-start !rounded-full !px-3" onConfigChange={(key, value) => onConfigChange(node.id, videoConfigPatch(key, value))} />
                        </>
                    ) : mode === "audio" ? (
                        <>
