...
export function InfiniteCanvas({ containerRef, viewport, backgroundMode = "lines", onViewportChange, onCanvasMouseDown, onCanvasDeselect, onCanvasDoubleClick, onContextMenu, onDrop, children }: InfiniteCanvasProps) {
    ...
    const handleWheel = useCallback(
        (event: React.WheelEvent<HTMLDivElement>) => {
            ...
            const baseViewport = nextViewportRef.current || viewportRef.current;
            const delta = -event.deltaY;
            const factor = Math.pow(1.1, delta / 100);
            const newScale = Math.min(Math.max(baseViewport.k * factor, 0.05), 5);
            const rect = containerRef.current?.getBoundingClientRect();
            if (!rect) return;

            const mouseX = event.clientX - rect.left;
            const mouseY = event.clientY - rect.top;
            const worldX = (mouseX - baseViewport.x) / baseViewport.k;
            const worldY = (mouseY - baseViewport.y) / baseViewport.k;
        ...
        [containerRef, scheduleViewportCommit],
    );
    ...
    const handlePointerDown = useCallback(
        (event: React.PointerEvent<HTMLDivElement>) => {
            ...
            if (event.button === 1 || (event.button === 0 && !isSpacePressedRef.current && isBackgroundClick)) {
            ...
            if (event.button === 0 && isSpacePressedRef.current && isBackgroundClick) {
                event.preventDefault();
            }
        },
        [onCanvasMouseDown],
