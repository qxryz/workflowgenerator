...
export function CanvasConfigComposer({ value, inputs, onChange, onClose }: CanvasConfigComposerProps) {
    ...
    const selectedReferenceIds = useMemo(() => new Set(tokens.flatMap((token) => token.type === "reference" ? [token.nodeId] : [])), [tokens]);
    ...
    useEffect(
        () => () => {
            ...
            if (pendingValueRef.current !== lastCommittedRef.current) onChangeRef.current(pendingValueRef.current);
        },
        [],
    );

    const commitEditorValue = (next: string, immediate = false) => {
    ...
    const insertReference = (input: NodeGenerationInput) => {
        ...
        closeMention();
        commitEditorValue(serializeEditor(editor));
    };
        ...
        >
            <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-baseline gap-2">
                    <div className="shrink-0 text-xs font-semibold">这一步要做什么</div>
                    <div className="truncate text-[11px] opacity-55">先选素材，再描述想得到的结果</div>
                </div>
                <Button size="small" type="text" className="!h-7 !w-7 !min-w-7 !p-0" icon={<X className="size-3.5" />} onClick={onClose} />
            </div>
            {inputs.length ? (
                <div className="thin-scrollbar mb-2 flex items-center gap-1.5 overflow-x-auto pb-0.5" aria-label="可用的上游素材">
                    <span className="mr-0.5 shrink-0 text-[11px]" style={{ color: theme.node.muted }}>可用素材</span>
                    {inputs.map((input) => {
                        const selected = selectedReferenceIds.has(input.nodeId);
                        return (
                            <button
                                key={input.nodeId}
                                type="button"
                                className="inline-flex h-7 max-w-36 shrink-0 items-center gap-1.5 rounded-md border px-2 text-[11px] transition-colors disabled:cursor-default"
                                style={{ borderColor: selected ? theme.node.activeStroke : theme.node.stroke, background: selected ? theme.toolbar.activeBg : theme.node.panel, color: theme.node.text }}
                                disabled={selected}
                                ...
                                onClick={() => insertReference(input)}
                            >
                                <span className="truncate">{resourceLabel(input, inputs)}</span>
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="relative rounded-xl">
                {!value.trim() ? <div className="pointer-events-none absolute left-3 top-2 text-sm leading-7" style={{ color: theme.node.placeholder }}>描述想得到的结果</div> : null}
                <div
                    ref={editorRef}
                    contentEditable
                    ...
                    onBlur={() => {
                        const editor = editorRef.current;
                        if (editor) commitEditorValue(serializeEditor(editor), true);
