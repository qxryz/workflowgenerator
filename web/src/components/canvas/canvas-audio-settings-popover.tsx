...
export function CanvasAudioSettingsPopover({ config, onConfigChange, buttonClassName, placement = "topLeft" }: CanvasAudioSettingsPopoverProps) {
    ...
    const voiceLabel =
        model === "speech-2.8-hd" || model === "speech-2.8-turbo"
            ? config.audioVoice && config.audioVoice !== "alloy"
                ? config.audioVoice
                : "male-qn-qingse"
            : model.startsWith("qwen-audio-") || model.startsWith("qwen3-tts")
              ? config.audioVoice && config.audioVoice !== "alloy"
                  ? config.audioVoice
                  : model.startsWith("qwen-audio-")
                    ? "longanhuan_v3.6"
                    : "Cherry"
            ...
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={buttonClassName || "!h-8 !max-w-[170px] !justify-start !rounded-full !px-2.5"} style={{ background: theme.node.fill, color: theme.node.text }} icon={<Settings2 className="size-3.5" />} onClick={() => setOpen((current) => !current)}>
                    <span className="truncate">
                        {voiceLabel} · {audioFormatLabel(config.audioFormat)} · {audioSpeedLabel(config.audioSpeed)}
                    </span>
                </Button>
            </span>
            {panel}
