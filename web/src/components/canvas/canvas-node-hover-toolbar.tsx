...
export function CanvasNodeHoverToolbar({
    ...
    const imageToolsTouchedRef = useRef(false);
    const { message } = App.useApp();
    const copyText = useCopyText();

    useEffect(() => {
        let active = true;
        void readUserPreference(IMAGE_QUICK_TOOLS_STORAGE_KEY).then((stored) => {
            if (!active || imageToolsTouchedRef.current || stored == null) return;
    ...
    const isTerminal = node.type === CanvasNodeType.Terminal;
    const isResultSlot = node.metadata?.role === "result-slot";
    const canOpenDialog = isText || hasImage || isVideo || isTerminal;
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(canOpenDialog && !isTerminal ? [{ id: "edit", title: "编辑", label: "编辑", icon: <MessageSquare className="size-4" />, onClick: () => onToggleDialog(node) }] : []),
        ...
        ...(isTerminal ? [{ id: "terminalSettings", title: "终端设置", label: "终端设置", icon: <Settings2 className="size-4" />, onClick: () => onToggleDialog(node) }] : []),
    ...
    const toolbarTools = isResultSlot ? baseToolbarTools : hasImage ? [...baseToolbarTools, ...nodeToolbarTools].filter((tool) => quickImageToolIdSet.has(tool.id as ImageQuickToolId)) : [...baseToolbarTools, ...nodeToolbarTools, ...extraTools];
    ...
    const saveImageToolSettings = () => {
        ...
        imageToolsTouchedRef.current = true;
        setQuickImageToolIds(config.ids);
        setShowImageToolLabels(config.showLabels);
        void saveUserPreference(IMAGE_QUICK_TOOLS_STORAGE_KEY, config);
                                        ...
                                        : node.type === CanvasNodeType.Image
                                          ...
                                          : node.type === CanvasNodeType.Video
                                            ...
                                            : node.type === CanvasNodeType.Audio
                                              ? "音频"
                                              : node.type === CanvasNodeType.Terminal
                                                ? "终端 Agent"
