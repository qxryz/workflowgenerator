...
export function PromptCard({
    ...
    return (
        <Card
            hoverable
            className="wg-library-card overflow-hidden"
            styles={{ body: { padding: 0 } }}
            cover={
                <button type="button" className="block w-full text-left" onClick={onOpen}>
                    {item.coverUrl ? (
                        <img src={item.coverUrl} alt={item.title} className="aspect-[4/3] w-full object-cover" loading="lazy" />
                    ) : (
        ...
        >
            <button type="button" className="block w-full text-left" onClick={onOpen}>
                <div className="p-4">
                    ...
                    {item.collection === "author" ? <AuthorNote note={item.authorNote} className="mt-3" /> : null}
