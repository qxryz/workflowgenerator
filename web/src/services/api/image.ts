...
function miniMaxBillingMode(config: AiConfig): MiniMaxBillingMode {
    const mode = (config as AiConfig & { minimaxBillingMode?: MiniMaxBillingMode }).minimaxBillingMode;
    return mode === "token-plan" ? "token-plan" : "payg";
}

async function requestMiniMaxImages(config: AiConfig, prompt: string, count: number, referenceImage?: string, options?: RequestOptions) {
    assertMiniMaxCredentialMatches(miniMaxBillingMode(config), config.apiKey);
    const body = buildMiniMaxImageRequest(config.model, {
        prompt: withImagePromptPrefix(config, prompt),
        ratio: config.size,
        count,
        optimizePrompt: config.imageOptimizePrompt !== "false",
        watermark: config.imageWatermark === "true",
        responseFormat: isDesktopApp() ? "url" : "base64",
        ...(referenceImage ? { referenceImage } : {}),
    });
    const payload = await postImageJson<Parameters<typeof parseMiniMaxImageResponse>[0]>(config, buildMiniMaxEndpoint(config.baseUrl, "image"), body, options);
    return parseMiniMaxImageResponse(payload).map((dataUrl) => ({ id: nanoid(), dataUrl }));
}
...
function withImagePromptPrefix(config: AiConfig, prompt: string) {
    const prefix = config.imagePromptPrefix.trim();
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (script) {
        ...
        try {
            const result = await runModelPlugin({
                ...
                prompt: withImagePromptPrefix(requestConfig, prompt),
                images: [],
        ...
        try {
            const response = await axios.post<ImageApiResponse>(
                aiApiUrl(requestConfig, "/images/generations"),
                {
                    ...
                    prompt: withImagePromptPrefix(requestConfig, prompt),
                    n: Math.min(10, n),
                    aspect_ratio: xaiAspectRatio(config.size),
                    resolution: xaiImageResolution(config.quality),
    ...
    try {
        const providerPayload =
            requestConfig.apiFormat === "ark"
                ? arkImageGenerationParameters(requestConfig.model, requestSize, n, config.imageWatermark === "true", config.imageOptimizePrompt !== "false")
                : {
                      n,
                      ...(quality ? { quality } : {}),
                      ...(requestSize ? { size: requestSize } : {}),
                      ...(background ? { background } : {}),
                      ...(outputParameters.responseFormat ? { response_format: outputParameters.responseFormat } : {}),
                      ...(outputParameters.outputFormat ? { output_format: outputParameters.outputFormat } : {}),
                  };
        const response = await postImageJson(requestConfig, aiApiUrl(requestConfig, "/images/generations"), { model: requestConfig.model, prompt: withImagePromptPrefix(requestConfig, prompt), ...providerPayload }, options);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
        ...
        try {
            const response = await axios.post<ImageApiResponse>(
                aiApiUrl(requestConfig, "/images/edits"),
                {
                    model: requestConfig.model,
                    prompt: withImagePromptPrefix(requestConfig, requestPrompt),
                    ...(refs.length === 1 ? { image: { type: "image_url", url: refs[0] } } : { images: refs.map((url) => ({ type: "image_url", url })) }),
                    aspect_ratio: xaiAspectRatio(config.size),
                    resolution: xaiImageResolution(config.quality),
                    response_format: "b64_json",
                },
    ...
    formData.set("prompt", withImagePromptPrefix(requestConfig, requestPrompt));
    formData.set("n", String(n));
    if (outputParameters.responseFormat) formData.set("response_format", outputParameters.responseFormat);
    if (outputParameters.outputFormat) formData.set("output_format", outputParameters.outputFormat);
    if (quality) {
...
function xaiAspectRatio(value: string) {
    ...
    if (XAI_IMAGE_RATIOS.includes(value)) return value;
    const dimensions = parseImageDimensions(value);
    if (!dimensions) return "auto";
    const target = dimensions.width / dimensions.height;
    return XAI_IMAGE_RATIOS.reduce((best, item) => {
        const ratio = parseRatioValue(item);
        const current = ratio.width / ratio.height;
        const bestRatio = parseRatioValue(best);
        return Math.abs(current - target) < Math.abs(bestRatio.width / bestRatio.height - target) ? item : best;
    }, "1:1");
}

function xaiImageResolution(value: string) {
    const normalized = value.toLowerCase();
    return normalized === "2k" || normalized === "medium" || normalized === "high" || normalized === "hd" || normalized === "4k" ? "2k" : "1k";
}

export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    const requestConfig = resolveModelRequestConfig(config, config.model || config.textModel);
    if (isMiniMaxAdapter(requestConfig.adapter)) {
        return requestMiniMaxTextReply(requestConfig, messages, onDelta, options);
    }
...
/** OpenAI-compatible providers frequently implement Chat Completions but not
 * Responses. A saved custom script can still target Responses, so recover from
 * a 404 by using the broadly supported endpoint for text-only node output. */
function shouldFallbackToChatCompletions(config: AiConfig, error: unknown) {
    ...
    return /(?:status(?:\s+code)?|http)\s*404|\b404\b/iu.test(message);
}

function toCompatibleChatMessages(config: AiConfig, messages: AiTextMessage[]) {
    const systemPrompt = config.systemPrompt?.trim();
    return systemPrompt ? [{ role: "system" as const, content: systemPrompt }, ...messages] : messages;
}

async function requestCompatibleChatText(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    if (!response.body) {
        ...
        const text = typeof content === "string" ? content : Array.isArray(content) ? content.map((item) => item.text || "").join("") : "没有返回内容";
        onDelta(text);
        return text;
    }
    ...
    const consume = (flush = false) => {
        ...
        for (const block of blocks) {
            ...
            const payload = JSON.parse(raw) as { choices?: Array<{ delta?: { content?: string | Array<{ text?: string }> } }>; error?: { message?: string } };
            if (payload.error?.message) throw new Error(payload.error.message);
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">) {
    try {
        ...
        const payload: NativeModelListPayload = isDesktopApp()
            ? (await fetchDesktopModelList(url, config.apiKey)) || {}
            : (
                  await axios.get<NativeModelListPayload>(url, {
                      headers: {
                          Authorization: `Bearer ${config.apiKey}`,
                      },
                  })
...
export async function fetchChannelModels(channel: ModelChannel) {
    if (channel.vendor === "minimax-token-plan") assertMiniMaxCredentialMatches("token-plan", channel.apiKey);
    if (channel.vendor === "minimax-api") assertMiniMaxCredentialMatches("payg", channel.apiKey);
