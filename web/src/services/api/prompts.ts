...
import type { PromptCollection, PromptSource } from "./prompt-source-presets";
...
export type PromptCollectionFilter = PromptCollection | "all";
export const PROMPT_COLLECTION_OPTIONS: { label: string; value: PromptCollectionFilter }[] = [
    { label: ALL_PROMPTS_OPTION, value: "all" },
    { label: "公开收录", value: "public" },
    { label: "作者私藏", value: "author" },
];
...
const promptCacheStore = createDesktopJsonStore({
    namespace: "prompt-cache-v1",
    legacy: { name: "infinite-canvas", storeName: "prompt_cache" },
});
const loadingSources = new Map<string, Promise<PromptSourceRefreshResult>>();
export const INSTALLED_AUTHOR_PROMPT_SOURCE_ID = "author-library-installed";
...
async function getAllPrompts(collection: PromptCollectionFilter): Promise<Prompt[]> {
    const settled = await Promise.all(
        enabledSources()
            .filter((source) => collection === "all" || source.collection === collection)
            .map(async (source) => {
                try {
    ...
    const installedAuthorPrompts = collection === "public" ? [] : getInstalledAuthorPrompts();
    return [...installedAuthorPrompts, ...settled.flat()];
}
...
export async function fetchPrompts({
    ...
    page = 1,
    pageSize = 20,
}: { keyword?: string; tag?: string[]; category?: string; collection?: PromptCollectionFilter; page?: number; pageSize?: number } = {}) {
    const items = await getAllPrompts(collection);
    const normalizedKeyword = keyword.trim().toLowerCase();
    const normalizedPage = Math.max(1, page);
    const normalizedPageSize = Math.max(1, Math.min(100, pageSize));
    const withoutTagFilter = filterPrompts(items, { keyword: normalizedKeyword, category, collection, tags: [] });
    const filtered = filterPrompts(items, { keyword: normalizedKeyword, category, collection, tags: tag });
    const categories = enabledSources()
        .filter((source) => collection === "all" || source.collection === collection)
        .map((source) => source.name);
    if (collection !== "public" && getInstalledAuthorPrompts().length) categories.unshift("作者私藏");
...
function getInstalledAuthorPrompts(): Prompt[] {
    return useAuthorPromptStore.getState().prompts.map((item) => ({
        ...item,
        sourceId: INSTALLED_AUTHOR_PROMPT_SOURCE_ID,
        category: "作者私藏",
        githubUrl: item.contentUrl,
        collection: "author",
    }));
}
...
function filterPrompts(items: Prompt[], options: { keyword: string; category: string; collection: PromptCollectionFilter; tags: string[] }) {
    return items.filter((item) => {
        if (options.collection !== "all" && item.collection !== options.collection) return false;
        ...
        return [item.title, item.prompt, item.description, item.authorNote, item.category, ...item.tags].filter(Boolean).join(" ").toLowerCase().includes(options.keyword);
    });
}
