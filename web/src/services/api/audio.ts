...
import { audioMimeType, normalizeAudioFormatValue, normalizeAudioSpeedValue, normalizeAudioVoiceValue, openAiAudioVoiceRequestValue } from "@/lib/audio-generation";
...
import { requestMiniMaxSpeech } from "./minimax-audio";
import { runModelPlugin } from "./model-plugin";
import { requestQwenSpeech } from "./qwen-audio";
...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<Blob | string> {
    ...
    if (isMiniMaxAdapter(requestConfig.adapter)) {
        const configuredVoice = config.audioVoice.trim();
        const miniMaxFormat = format === "wav" || format === "flac" ? format : "mp3";
        return requestMiniMaxSpeech(
            config,
            config.model || config.audioModel,
            {
                text: prompt,
                voiceId: configuredVoice && configuredVoice !== "alloy" ? configuredVoice : "male-qn-qingse",
                format: miniMaxFormat,
                sampleRate: 32_000,
                speed: Math.max(0.5, Math.min(2, Number(config.audioSpeed) || 1)),
            },
            options?.signal,
        );
    }
    if (requestConfig.apiFormat === "qwen" || /^(?:qwen(?:3|-audio)-.*tts)/i.test(model)) {
        const configuredVoice = config.audioVoice.trim();
        const voice = model.startsWith("qwen-audio-") ? (configuredVoice && configuredVoice !== "alloy" ? configuredVoice : "longanhuan_v3.6") : configuredVoice && configuredVoice !== "alloy" ? configuredVoice : "Cherry";
        return requestQwenSpeech(
            config,
            config.model || config.audioModel,
            {
                text: prompt,
                voice,
                format: normalizeAudioFormatValue(config.audioFormat) === "wav" ? "wav" : "mp3",
                sampleRate: 24_000,
                instruction: config.audioInstructions,
                rate: Math.max(0.5, Math.min(2, Number(config.audioSpeed) || 1)),
            },
            options?.signal,
        );
    }
    ...
    try {
        const response = await axios.post<Blob>(
            aiApiUrl(requestConfig, "/audio/speech"),
            {
                model,
                input: prompt,
                voice: openAiAudioVoiceRequestValue(config.audioVoice),
...
export async function storeGeneratedAudio(blob: Blob | string, format = "mp3"): Promise<UploadedFile> {
    if (typeof blob === "string") return uploadMediaFile(blob, "audio");
