...
async function fetchSource(source: PromptSource, options?: RunOptions) {
    const urls = Array.from(new Set([source.url, source.fallbackUrl].filter((url): url is string => Boolean(url?.trim()))));
    ...
    throw lastError instanceof Error ? lastError : new Error("来源不可用");
}
...
function normalizeItems(values: unknown[], source: PromptSource) {
    ...
    values.forEach((value, index) => {
        ...
        items.push({
            ...
            authorNote: optionalString(record.authorNote),
            coverUrl,
            referenceImageUrls,
