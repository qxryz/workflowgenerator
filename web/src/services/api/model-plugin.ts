 ...
 * `/v1` usable as well, instead of sending a broken `/v1/v1/...` request. */
function normalizePluginUrl(value: string) {
    try {
        const url = new URL(value);
        url.pathname = url.pathname
            .replace(/\/v1\/v1(?=\/|$)/giu, "/v1")
            .replace(/\/v1beta\/v1beta(?=\/|$)/giu, "/v1beta");
        return url.toString();
    } catch {
        return value;
    }
...
function createPluginHttp(config: AiConfig, capability: ModelCapability, options?: RequestOptions): PluginHttp {
    const run = async (method: "get" | "post", path: string, body: unknown, opts?: PluginHttpOptions) => {
        const payload = capability === "image" ? sanitizeImageRequestPayload(config.apiFormat, config.model, body) : body;
...
function createPluginRequest(config: AiConfig, capability: ModelCapability, options?: RequestOptions) {
    return async (requestConfig: AxiosRequestConfig & { url: string }) => {
        const data = capability === "image" ? sanitizeImageRequestPayload(config.apiFormat, config.model, requestConfig.data) : requestConfig.data;
        const response = await axios.request({ ...requestConfig, data, url: pluginUrl(config, requestConfig.url), signal: options?.signal });
        return response.data;
    };
}
...
return { data: audio.data };`,
    ...
    text: [
        {
            label: "OpenAI 规范",
            script: `// 文本对话（OpenAI 兼容 Chat Completions）。可用：messages([{role,content}])、model、reasoningEffort
const data = await request({
  method: "post",
