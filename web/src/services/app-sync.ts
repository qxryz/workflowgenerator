...
export async function syncAppDataToWebdav(config: WebdavSyncConfig, onProgress?: AppSyncProgress): Promise<AppSyncResult> {
    ...
    const [canvas, assets] = await Promise.all([
        syncDomain<CanvasDomainData>(config, onProgress, {
            ...
            mergeData: (local, remote) => ({ projects: mergeById(local.projects, remote.projects.map(portableCanvasProject), "updatedAt") }),
            applyData: async (data) => useCanvasStore.getState().replaceProjects(data.projects),
            portableData: (data) => ({ projects: data.projects.map(portableCanvasProject) }),
        }),
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    let unregisterMergedData: (() => void) | undefined;
    try {
        ...
        unregisterMergedData = registerRuntimeMediaReferenceProvider(() => mergedData);
        ...
        const manifest: DomainManifest<T> = {
            ...
            data: options.portableData?.(mergedData) || mergedData,
...
async function hydrateAsset(asset: Asset): Promise<Asset> {
    if (asset.kind === "image" && asset.data.storageKey) {
        ...
        return { ...asset, coverUrl: shouldRefreshStoredAssetCover(asset.coverUrl, asset.data.dataUrl) ? dataUrl : asset.coverUrl, data: { ...asset.data, dataUrl } };
    }
