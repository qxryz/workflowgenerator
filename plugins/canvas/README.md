...
WorkflowGenerator 官方插件由 CI 构建并发布到仓库的 `plugins-dist` 分支（构建产物不进入 `main`）。画布“节点插件”面板从该分支读取并一键安装；第三方插件仍使用用户填写的 JS URL。构建脚本与发布说明见 [`registry/`](./registry/README.md)，私有部署可用 `VITE_PLUGIN_REGISTRY_URL` 覆盖来源。
...
| `ctx.media` | 通过宿主媒体存储保存、恢复或回滚图片；不要把大体积 data URL 直接写入节点 metadata |
